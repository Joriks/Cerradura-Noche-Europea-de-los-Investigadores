/**
 * Author: Jorge Bote Albala
 * Author: Francisco Javier Castaño Mendo
 */
    ...
    for(int i = 0;i<LIST_MAX;i++){
      if(kpd.key[i].stateChanged && kpd.key[i].kstate == PRESSED)
        switch(kpd.key[i].kchar){
            case 'A':
              Serial.println("Altas");
              getFingerprintEnroll(6);
              break;
            case 'B':
              Serial.println("Bajas");
              //dar de baja
              break;
            default:
             // Serial.println("Waiting for valid finger...");
              while(getFingerprintIDez() != FINGERPRINT_OK);
